...
public class NoteBookWindow extends Application {
	...
	public void start(Stage stage) {
		...
		saveNote.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				...
				if (currentFolder == "-----" || currentNote == "") {
					...
					Folder f = noteBook.getFolders().get(
							noteBook.getFolders().indexOf(
									new Folder(currentFolder)));
					TextNote n = new TextNote(currentNote, textAreaNote
							.getText());
					f.getNotes().remove(n);
					f.getNotes().add(n);
				}
		...
		ImageView saveImg = new ImageView(new Image(new File("save.png")
				.toURI().toString()));
		...
		ImageView delImg = new ImageView(new Image(new File("delete.png")
				.toURI().toString()));
	...
	private HBox addHBox() {
		...
		buttonLoad.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent arg0) {
				...
				fileChooser
						.setTitle("Please Choose An File Which Contains a NoteBook Object!");
		...
		searchLabel
				.setStyle("-fx-font-size: 15px; -fx-font-family: Courier, Arial");
		...
		searchBtn.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				...
				if (folder != null) {
					ArrayList<Note> searchResult = new ArrayList<Note>(folder
							.searchNotes(currentSearch));
		...
		hbox.getChildren().addAll(buttonLoad, buttonSave, searchLabel,
				searchBox, searchBtn, clearSearchBtn);
	...
	private void updateNoteBookInfo() {
		...
		foldersComboBox.getSelectionModel().selectedItemProperty()
			...
			public void handle(ActionEvent event) {
				...
				if (result.isPresent()) {
					...
					if (result.get().isEmpty()
							|| noteBook.getFolders().contains(
									new Folder(result.get()))) {
						Alert alert = new Alert(AlertType.WARNING);
						alert.setTitle("Warning");
						if (result.get().isEmpty())
							alert.setContentText("Please input a valid folder name");
						else
							alert.setContentText("You already have a folder named with "
									+ result.get());
						alert.showAndWait();
					} else {
				...
				if (currentFolder == "" || currentFolder == "-----") {
					...
					if (result.isPresent()) {
						...
						if (result.get().isEmpty()) {
							...
							if (!noteBook.createTextNote(currentFolder,
									result.get())) {
								Alert alert = new Alert(AlertType.WARNING);
								alert.setTitle("Warning");
								alert.setContentText("You already have a note named with "
										+ result.get());
								alert.showAndWait();
							} else {
								updateListView();
								...
								alert.setContentText("Insert " + result.get()
	...
	private void updateListView() {
		...
		ObservableList<String> combox2 = FXCollections
	...
	private void loadNoteBook() {
		...
		nb.createTextNote("COMP3021", "COMP3021 syllabus",
		...
		nb.createTextNote("Holiday", "Los Angeles",
				"Peter said he wants to go next Agugust");
		nb.createTextNote("Holiday", "Christmas",
